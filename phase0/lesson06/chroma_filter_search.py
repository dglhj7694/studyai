...
def print_results(query: str, category: str, results: list[dict]) -> None:
    print("=" * 80)
    print(f"질문: {query}")
...
def main():
    output_path = "phase0/lesson06/outputs/chroma_filter_search_results.json"

    query = input("질문을 입력하세요: ").strip()
    category = input("검색할 category를 입력하세요. 예: rag, fine_tuning, transformer: ").strip()
    ...
    if not category:
        print("category가 비어 있습니다.")
        return

    raw_results = search_chroma_with_filter(
        query=query,
        category=category,
        n_results=5,
    )
    ...
    print_results(query, category, results)
