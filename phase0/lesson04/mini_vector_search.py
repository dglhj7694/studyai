...
def print_results(query_name: str, query_vector: list[float], results: list[dict]) -> None:
    print("=" * 80)
...
def main():
    ...
    last_results = search_by_vector(
        query_vector=queries[0]["vector"],
        documents=documents,
        top_k=3,
    )
    save_results(last_results, output_path)

    print(f"마지막 검색 결과 저장 완료: {output_path}")
